...
> 目的：把 Ch4 §4.2 里 skin.shpk 的 60 项 MaterialParam 逐一认名。初始状态下有 33 个 CRC 未识别，本附录把其中 **17 个** 通过交叉比对第三方工具（Meddle、ALum、xivmodding）认领成功，**16 个** 暂无可靠命名但给出默认值与用途线索。
> 方法：
> 1. 先本地用 FFXIV 专用 CRC（多项式 `0x04C11DB7`、输入 bit-reflect、输出 bit-reflect32）对候选名字批量算哈希（见 §4a.4 Python 代码）
