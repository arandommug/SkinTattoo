...
> 分类：1️⃣ 发光系 · 2️⃣ PBR 扩展 · 3️⃣ 接缝/兼容性工程 · 4️⃣ 全新渲染特性 · 5️⃣ 插件层"不改 shader"玩法。
...
| **风险** | ⚠️低 / 中 / 高 | 低=改坏只影响发光 / 中=可能影响所有皮肤渲染 / 高=随机花屏或崩溃 |
...
- **风险**：⚠️ **低**（仅影响 specular mask 计算，不动光照模型）
...
- **风险**：⚠️ **中**（可能让 emissive 过曝，影响整个 tone mapping）
...
- **风险**：⚠️ **高**（Block E1 是 244 条指令的复杂 SSS 管线，动一条牵连全身）
...
- **风险**：⚠️ **高**（这 6 个数是 SE 调过的 SSS 曲线，乱改会导致皮肤发蓝/发绿）
...
- **风险**：⚠️ **高**（RDEF 改动失败会让整个 Body 渲染崩溃）
...
- **风险**：⚠️ **中**（可能找不到合适的 varying，需要 VS 侧也改）
...
- **风险**：⚠️ **中**（新 slot 未被其他 PS 占用，但要注意 mtrl 写入时的 tex 索引）
...
| 章节 | 状态 | 交付文件 |
|---|---|---|
| Ch0 总览 | ✅ | `00-overview.md` |
| Ch1 shpk 结构 | ✅ | `01-shpk-structure.md` |
| Ch2 SkinType 分支 | ✅ | `02-skintype-branches.md` |
| Ch3 DXBC 对比 | ✅ | `03-dxbc-body-vs-emissive.md` |
| Ch4 cbuffer/sampler/texture | ✅ | `04-cbuffers.md` |
| Ch4a MaterialParam CRC 附录 | ✅ | `04a-materialparam-crc-appendix.md` |
| Ch5 接缝与改造路径 | ✅ | `05-seam-and-fix-paths.md` |
| Ch6 v1 PS[19] 逐段解剖 | ✅ | `06-ps19-anatomy.md` |
| Ch6 v2 Block E 细解 | ✅ | `06b-block-e-sss-ggx.md` |
| Ch7 高级效果 idea 清单 | ✅ | `07-advanced-mod-ideas.md`（本文）|
