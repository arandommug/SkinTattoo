...
> 提取工具：`SkinTatoo/ShaderPatcher/extract_ps.py`（本轮新写，详见 §3.6）
...
三种 SkinType 的 cbuffer 条目、slot、大小全部一致。这与 §2.3 里"pass[2] 所有 SkinType 共用 VS[2, 11, 20, ...]"呼应 —— **同一 VS 输出喂给不同的 PS，但 PS 的输入接口完全相同**。
...
整份 shader 里对 cb5 的使用只有**一条指令**（见 §3.4）。
