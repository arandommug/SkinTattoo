...
> 阅读前置：建议先过完 Ch6 v1 把握 r0..r11 各寄存器的含义；Block E 在进入前，PS[19] 的寄存器状态已准备完毕（r7=世界法线、r3=视向、r11=伪光向量、r8.xyz=F0、r9.xyz=albedo²、r6.xyz=skin 参数、r0.y=粗糙度之类）。
...
```
351  dp3 r4.w, v3.xyzx, v3.xyzx          ; |v3|²      v3 = VS 传来的 tangent
...
```
365  mad r9.w,  r6.y, l(-0.156583), l(1.570796)     ; r9.w  = π/2 - 0.156583 × r6.y
366  add r10.w, -r6.y, l(1.000000)                  ; r10.w = 1 - r6.y
367  sqrt r10.w, r10.w                              ; r10.w = √(1 - r6.y)
368  mad r9.w, -r9.w, r10.w, l(1.570796)            ; r9.w  = π/2 - r9.w × r10.w
                                                    ;       = arccos(r6.y) 的一种低阶近似
```
[B] `1.570796 = π/2`，`-0.156583 ≈ -π/2 × 0.0997` —— 这是 **Shanks / Abramowitz arccos 近似** 的一种变体：

```
arccos(x) ≈ π/2 - x × (1 + 0.156583 × √(1-x))
```

更确切地：SE 把它写成 `arccos(x) ≈ π/2 - (π/2 - 0.156583 × x) × √(1-x)`，展开后等价。
...
374  mad r10.w, -r10.w, r11.w, l(3.141593)          ; π - ...
375  movc r10.w, r13.y, r13.x, r10.w                ; 分支：T·L 正负选不同公式
376  add r10.w, -r10.w, l(1.570796)                 ; π/2 - r10.w
```
[B] `3.141593 = π`。这里处理了 `T·L` 正负号的对称性，等价于计算 `arcsin(T·L)` 而不是 arccos。
...
382  dp3 r10.w, r11.xyzx, r12.xzwx                  ; r10.w = (L⊥) · (V⊥)
383  dp3 r11.x, r11.xyzx, r11.xyzx
384  dp3 r11.y, r12.xzwx, r12.xzwx
385  mad r11.x, r11.x, r11.y, l(0.000100)           ; |L⊥|² × |V⊥|² + ε
386  rsq r11.x, r11.x                               ; 1 / √(...)
387  mul r10.w, r10.w, r11.x                        ; r10.w = cos(角度 between L⊥ 和 V⊥)
...
390  sqrt r11.x, r11.x                              ; r11.x = √sat(...)
```
...
393  mul r12.x, r0.y, r0.y                                  ; r12.x = roughness²
...
395  mad r13.xy, cb7[r1.w + 3].yzyy, r12.wwww, r0.yyyy
         ...
         ; 这是**各向异性粗糙度**：两个方向（y/z）各取一个 α_t, α_b
396  mul r13.xy, r13.xyxx, r13.xyxx                         ; r13.xy = α_t², α_b²
...
400  mad r13.w, -r6.y, r6.y, l(1.000000)                    ; r13.w = 1 - (T·V)²
401  sqrt r13.w, r13.w                                      ; r13.w = √(1-(T·V)²) = sin(angle TV)
402  mul r14.x, r6.y, r14.x                                 ; r14.x = (T·V) × sin(axis)
403  mad r13.z, r13.z, r13.w, r14.x                         ; r13.z = r13.z × sin(TV) + r14.x
404  mul r12.x, r11.x, r12.x                                ; r12.x = r11.x × α²
405  mul r14.xy, r12.xxxx, l(1.414214, 3.544908, 0.000000, 0.000000)
         ; r14.x = r12.x × √2
         ; r14.y = r12.x × √(4π) ≈ 3.54491
```
[B] `1.414214 = √2`, `3.544908 = √(4π) = √(12.566) ≈ 3.545` —— **Beckmann 分布归一化常数**。标准 Beckmann PDF 是
`D(m) = exp(-tan²θ/α²) / (π α² cos⁴θ)`，其归一化项 `1/(α π)` 和 `1/(α² π)` 对应这两个数。

[C] `cb7[r1.w + 3].yz` 是 **各向异性粗糙度 α_t, α_b**（tangent/bitangent 方向独立粗糙度）。`cb7[r1.w + 2].y` 是 **各向异性主轴旋转角**。这两条线索结合确认 Block E1 走的是**各向异性 BRDF**（普通皮肤走 E3，精细皮肤/脸部走 E1）。
...
408  mul r8.w, r8.w, r8.w                                   ; r8.w²
409  mul r8.w, r8.w, l(-0.500000)                           ; r8.w = -r8.w² / 2
410  mul r12.x, r14.x, r14.x                                ; r12.x = (√2 × ...)²
411  div r8.w, r8.w, r12.x                                  ; r8.w = -r8.w² / (2 × (√2α)²)
412  mul r8.w, r8.w, l(1.442695)                            ; × 1/ln(2)  → 准备给 exp2
413  exp r8.w, r8.w                                         ; r8.w = 2^(...)  = exp(...)
414  div r8.w, r8.w, r14.y                                  ; r8.w /= √(4π)   Beckmann 归一化
```
[A] 这段就是 `exp(-((T·V+T·L)-2sin)² / (4α²)) / √(4π α²)` —— **Beckmann 各向异性分布**。`1.442695 = 1/ln(2)` 是把 `exp(x)` 用 `exp2(x*1/ln2)` 替换（DXBC 优先 exp2 指令），最终数学值相同。
...
416  mad_sat r12.x, r6.w, l(0.500000), l(0.500000)          ; r12.x = sat(0.5 × V·L + 0.5)
                                                           ;        = sat((1 + V·L) / 2)
417  sqrt r12.x, r12.x                                      ; r12.x = √sat(...)
418  add r12.x, -r12.x, l(1.000000)                         ; r12.x = 1 - √sat(...)
419  mul r12.w, r12.x, r12.x                                ; r12.x²
420  mul r12.w, r12.w, r12.w                                ; r12.x⁴
421  mul r12.x, r12.x, r12.w                                ; r12.x × r12.x⁴ = r12.x⁵
422  mad r12.x, r12.x, l(0.953479), l(0.046521)             ; Fresnel: r12.x⁵ × 0.95347 + 0.04652
423  mul r8.w, r8.w, r12.x                                  ; × Fresnel
424  mul r8.w, r8.w, l(0.250000)                            ; / 4  (BRDF denominator 4cosV cosL 中的 0.25)
```
[A] **Schlick 菲涅尔 `F(h·v) = F₀ + (1-F₀)(1-cosθ)⁵`**，其中 `F₀ = 0.046521 ≈ 0.04`（IOR 1.5 的绝缘体，正是标准皮肤 F0）。
`0.953479 = 1 - 0.046521` 是 `(1 - F₀)` 展开。L422 这一行可以写成：

```
F = F₀ + (1 - F₀) × pow(1 - cosθ_half, 5)
  = 0.046521 + 0.953479 × r12.x⁵
```
...
430  mul r6.yw, r6.yyyw, l(0, -0.5, 0, -0.5)                ; -x²/2
431  mul r12.xw, r13.xxxy, r13.xxxy                         ; α_t⁴, α_b⁴
432  div r6.yw, r6.yyyw, r12.xxxw                           ; / α⁴
433  mul r6.yw, r6.yyyw, l(0, 1.442695, 0, 1.442695)        ; / ln(2)
434  exp r6.yw, r6.yyyw                                     ; exp
435  mul r12.xw, r13.xxxy, l(2.506628, 0, 0, 2.506628)      ; α² × √(2π) ≈ 2.5066
436  div r6.yw, r6.yyyw, r12.xxxw                           ; Gaussian 归一化
```
[B] `2.506628 = √(2π)` —— **Gaussian 的归一化常数** `1/√(2πσ²)`。这里做了**第二个 Gaussian**，中心点由 `cb7[r1.w + 2].zw` 给（LUT 里可配置的 SSS 峰值位置），方差由 α² 决定。
...
441  mad r11.x, -r10.w, r10.w, l(1.000000)                  ; 1 - r10.w²
442  max r11.x, r11.x, l(0.000000)
443  sqrt r11.x, r11.x                                      ; √sat(1 - r10.w²)
...
453  mad r1.w, -r1.w, r1.w, l(1.000000)                     ; 1 - (...)²
454  sqrt r1.w, r1.w                                        ; √
455  mul r1.w, r1.w, l(0.500000)                            ; × 0.5
456  div r1.w, r1.w, r9.w                                   ; / r9.w
457  log r13.xyz, r9.xyzx                                   ; log(albedo²) = 2×log(albedo)
...
```
[B] L457-459 是 **`pow(albedo², exp)`** 的经典 DXBC 写法（log + mul + exp），等价于
...
477  mul r12.xzw, r12.zzzz, r13.xxyz                         ; r12.xzw = power_exp × log(albedo²)
...
E1 输出 r11.xyz ≈ skinBRDF_E1(V, L, T, roughness, LUT)
             = Fresnel₁ × Beckmann_各向异性(T·V+T·L, 2α_t² α_b²)
               × wrap(V·L)
             + (1 - Fresnel₂)² × pow(albedo², 2r1.w)     ← SSS 吸收色 #1
             + Fresnel₃ × (1 - Fresnel₃)² × pow(albedo², 2r12.z) × 背光mask    ← SSS 吸收色 #2
```

- Fresnel 系数 `F₀ = 0.04652` (= 0.046521)、`1-F₀ = 0.95348` 出现至少 4 次
- Beckmann 各向异性：`cb7[LUT+3].yz` 是 `α_t, α_b`，`cb7[LUT+2].y` 是主轴角度
- 能量守恒：`pow(albedo², exp)` 出现 2 次，exp 来自 roughness 派生量
...
492    add r13.xyz, -r8.xyzx, l(1,1,1,0)                     ; r13 = 1 - F₀     (F₀ in r8)
493    add r2.z, -r2.z, l(1.000000)                          ; r2.z = 1 - VdotH
494    mul r4.w, r2.z, r2.z
495    mul r4.w, r4.w, r4.w
496    mul r2.z, r2.z, r4.w                                  ; (1-VdotH)⁵
497    mad r13.xyz, r13.xyzx, r2.zzzz, r8.xyzx
                                                            ; r13 = F₀ + (1-F₀)(1-VdotH)⁵
                                                            ;      = Schlick Fresnel（按 R/G/B 分别算）
498    dp3_sat r2.z, r7.xyzx, r12.xzwx                       ; r2.z = sat(N · H)    (NdotH)
```
[A] L492-497：**经典的 color-valued Schlick Fresnel**，`F(v·h) = F₀ + (1-F₀)(1-v·h)⁵`。`r13.xyz` 是**彩色** Fresnel（因为 F₀ 来自金属反射系数 albedo² 在 L341 已拟合成向量）。

```
499    mul r4.w, r0.y, r0.y                                  ; α = roughness²
500    max r4.w, r4.w, l(0.001000)                           ; α ≥ 0.001
501    mul r5.w, r4.w, r4.w                                  ; α²
502    max r6.y, r5.w, l(0.001000)                           ; ≥ 0.001
503    mul r2.z, r2.z, r2.z                                  ; NdotH²
504    mad r6.w, r4.w, r4.w, l(-1.000000)                    ; α² - 1
505    mad r6.w, r2.z, r6.w, l(1.000000)                     ; NdotH²(α²-1) + 1
506    mul r6.w, r6.w, r6.w                                  ; 平方
507    mul r6.w, r6.w, l(3.141593)                           ; × π
508    rcp r6.w, r6.w                                        ; 1 / denominator
509    mul r5.w, r5.w, r6.w                                  ; D(h) = α² / (π × (NdotH²(α²-1) + 1)²)
```
...
```
510    mul r6.w, r6.y, r6.y                                  ; α⁴   (smooth term)
511    mad r6.y, r6.y, r6.y, l(-1.000000)                    ; α⁴ - 1
512    mad r2.z, r2.z, r6.y, l(1.000000)                     ; NdotH²(α⁴-1)+1
513    mul r2.z, r2.z, r2.z
514    mul r2.z, r2.z, l(3.141593)
515    rcp r2.z, r2.z
516    mul r2.z, r2.z, r6.w                                  ; 另一个 GGX D? 用 α⁴ 的"高粗糙"版本
517    add r6.y, r0.y, l(1.000000)                           ; roughness + 1
518    mul r6.y, r6.y, r6.y
519    mul r6.w, r6.y, l(0.125000)                           ; (r+1)² / 8   ← Smith G k₁
520    mad r6.y, -r6.y, l(0.125000), l(1.000000)             ; 1 - (r+1)²/8
...
```
528-538：类似 519-527 的 Smith G 第二套计算（用 α⁴ 的版本，做能量补偿）
...
```
E2 输出 r11.xyz = GGX_spec(N, V, L, roughness, F₀)
             = D_GGX(NdotH, α²) × G_Smith(NdotV, NdotL, α) × F_Schlick(VdotH, F₀)
               / (4 × NdotV × NdotL)
```

完全是 **UE4 经典 BRDF**，没有 skin-specific 修正。这也是"body 切 Emissive 接缝"的一个隐藏因子：Body PS[8] 走 E2，Emissive PS[19] 也走 E2（当 skinID ≠ 1/3 时）——两者此处一致，所以接缝主要来自 Block A 早期（cb0[3]² 初始化）和 Block I 尾段（cb5[0] 乘法）。
...
556  add r12.xzw, -r8.xxyz, l(1, 0, 1, 1)                    ; 1 - F₀
557-560  Schlick (1-VdotH)⁵
561  mad r12.xzw, r12.xxzw, r0.zzzz, r8.xxyz                 ; F = F₀ + (1-F₀)(1-VdotH)⁵
...
594  mul r0.z, r3.w, l(3.141593)                             ; r0.z = π × NdotL
...
- **否则**乘 `π × NdotL` → 和标准 Lambert 系数合拍

这解释了 `LUT[0].x == 3` 是"**完全没有 specular 的特殊皮肤**"—— 比如可能是幽灵角色、涂白漆的 NPC 之类。

`r10.xyz` 自此作为 specular 输出参与 Block F（Ch6 v1 §6.7 已覆盖）。
...
| 概念 | 确定性 | 对应指令 |
|---|---|---|
| GGX 分布 D(h) = α²/(π(NdotH²(α²-1)+1)²) | A | L499-509（E2）, L564-573（E3） |
| Smith G = NdotV/(NdotV(1-k)+k) × NdotL/(NdotL(1-k)+k), k=(r+1)²/8 | A | L517-527（E2）, L574-584（E3） |
| Schlick F = F₀ + (1-F₀)(1-VdotH)⁵ | A | L492-497（E2）, L556-561（E3）, L415-422/468-473（E1 四次） |
| Beckmann 各向异性 | A | L406-414（E1） |
| Gaussian SSS 衰减 | B | L428-436（E1） |
| arccos/arcsin 近似（Abramowitz） | B | L365-379（E1） |
| 能量守恒 pow(albedo², exp) | A | L457-460/477-479（E1） |
| skinID LUT[0].x == 3 → 无 specular | A | L594-595 |
| 未拟合的 SSS 多项式常数 (0.5, -3.65, 17.0) 等 | C | L388（E1） |
| **F₀ = 0.046521 (= 0.04)** 皮肤 IOR 1.5 典型值 | A | 所有 Fresnel 常数 |
...
| 改 F₀ 让皮肤更金属 | 把 L422 的 `0.046521` 改为更大值（如 0.5） | 中 — 视觉会明显"金属化" |
