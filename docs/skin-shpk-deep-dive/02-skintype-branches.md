...
> 解析工具：`SkinTatoo/ShaderPatcher/dump_nodes.py`（本轮新写，详见 §2.6）
...
- **NodeAlias=22272**：22 k 条 selector→node 别名，使得引擎对同一物理 PS 可以通过大量 key 组合命中。这是 §1.4 `NodeSelectors` 字典把所有枚举展平的结果。
- **Nodes=768**：这是 shpk 真正"物理"的变体条目数，768 正好对应 `5 SkinType × 8 SceneKey × ??? = 768`（实际分布见 §2.2）。
...
1. **ValBody 节点数是别人的两倍**（256 vs 128）。这解释了 §1.2 里 `pass[0]` 在 Body 下出现 96 对 `(VS, PS)` 独立组合、而其他分支只有 32 对的差异 —— Body 多了一整组并列变体（推测与 Hrothgar 身体毛发相关，虽然 SystemKey 为空，但可能走的是额外 SceneKey 维度）。
...
完整 pass 分布见 `dump_nodes.py` 输出（§2.6 有复现方式）。摘要：
...
2. **Body 多出来的 128 个节点走哪条 key？** 四个 key 类别 (3 Mat + 2 SubView + 0 Sys + 8 Scene) 应该只能给 Body 分出 128 种组合，却实际有 256。说明 Penumbra 在 Ch1 §1.4 末尾提到的"SubViewKey 值由运行时 fill"之外，Body 还多了一条隐式 key。
