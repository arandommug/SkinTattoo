...
### 物证 1：vanilla 时 Body 与 Face 的 pass[2] PS 本就不同（Ch2 Sec.2.4）
...
### 物证 2：Body 专属的"normal.alpha gloss mask"链（Ch4 Sec.4.4）
...
### 物证 4：Emissive PS[19] 采了 normal.alpha 但**把它挪去做 emissive mask**（Ch4 Sec.4.6）

```
sample_b_indexable r0.xz, v2.xyxx, t5.zxwy, s1, r0.x   ; 同 Body：r0.x=normal.z, r0.z=normal.alpha
mul r1.xyz, cb0[3].xyzx, cb0[3].xyzx                   ; r1.xyz = g_EmissiveColor^2
mul r1.xyz, r0.zzzz, r1.xyzx                           ; r1.xyz = g_EmissiveColor^2 * normal.alpha
...
| 行号 | 指令 | 作用 |
|---:|---|---|
| 296 | `mul r1.xyz, cb0[3].xyzx, cb0[3].xyzx` | `r1 = g_EmissiveColor^2`（sRGB -> 近似线性） |
...
**核心洞察**：emissive 分量最终的贡献 ~= `g_EmissiveColor^2 * normal.alpha * m_EmissiveColor_dynamic * max(luma, 1) * m_MulColor`。亮度被多次放大，其中 `max(luma, 1)` 尤其关键 ---- 它让 emissive **在本就亮的区域变得更亮**，这就是我们 UI 上感觉"发光强度不线性"的根本原因。
...
**好消息**：
- Ch4 Sec.4.1 已经确认 Body PS 的 cbuffer slot 0..6 被占用，slot 7+ 还空着 ---- 可以把 `g_MaterialParameterDynamic` 绑到 b7
- sampler 用的是 s0-s4，t0-t9；s5 / t10 未占用 ---- 和我们 PS[19] patch 用的相同 slot 方案
- Body PS 的结构和 Emissive PS 非常接近（Ch3 Sec.3.2 七个 cbuffer 完全一致），reg alloc 几乎对齐
...
### 新认识（Ch3 Sec.3.3、Ch4 Sec.4.4 之后）
...
- [x] r1.xyz 的 emissive 通路全程可追踪（Ch5 Sec.5.2）
