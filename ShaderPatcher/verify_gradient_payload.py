"""Sanity-check: parse the raw gradient DXBC payload bytes embedded in SkinShpkPatcher.cs
and verify every instruction's (opcode, length) is self-consistent.

Catches encoding bugs like wrong length bytes or operand count mismatches that would
cause the GPU to reject the patched shader (DXGI 11000002 / device hung).
"""
import struct

# Full gradient payload tokens, in C# byte order (little-endian words).
# Copy-paste from SkinShpkPatcher.cs PulsePayload arg list.
TOKENS_HEX = """
...
OPCODE_NAMES = {
    0x00: "add", 0x1D: "ge", 0x32: "mad", 0x36: "mov", 0x37: "movc",
    0x38: "mul", 0x45: "sample", 0x4D: "sincos",
}

tokens = [int(t, 16) for t in TOKENS_HEX.split()]
print(f"Total tokens: {len(tokens)} ({len(tokens)*4} bytes)\n")
...
while pos < len(tokens):
    op_token = tokens[pos]
    opcode = op_token & 0x7FF
    length = (op_token >> 24) & 0x7F
    ext_bit = (op_token >> 31) & 1
    if length == 0: length = 1
    name = OPCODE_NAMES.get(opcode, f"op_0x{opcode:02X}")
    tokens_slice = tokens[pos:pos + length]
    actual_tokens = ' '.join(f'0x{t:08X}' for t in tokens_slice)
    print(f"  [{inst_idx:2}] pos={pos:3} opcode=0x{opcode:02X}({name:6}) len={length:2} ext={ext_bit} | {actual_tokens}")
    if pos + length > len(tokens):
        print(f"      !! OVERRUN: instruction claims {length} tokens but only {len(tokens) - pos} remain")
        err_count += 1
        break
    pos += length
    inst_idx += 1

if err_count == 0 and pos == len(tokens):
    print(f"\n[OK] All {inst_idx} instructions self-consistent, exactly {len(tokens)} tokens.")
else:
    print(f"\n[FAIL] {err_count} errors, stopped at pos={pos}/{len(tokens)}.")
